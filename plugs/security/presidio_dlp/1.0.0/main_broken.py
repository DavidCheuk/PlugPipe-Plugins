...
def process(ctx, cfg):
    """
    PlugPipe entry point for Presidio DLP plugin with timeout protection
    ...
    try:
        try:
            # Validate Presidio availability
            if not PRESIDIO_AVAILABLE:
                return {
                    "status": "warning",
                    "message": "Presidio not available - using fallback patterns",
                    "install_command": "pip install presidio-analyzer presidio-anonymizer",
                    "fallback_mode": True
                }
            ...
            # Quick pattern-based fallback for timeout cases
            def quick_pii_scan(text):
                import re
                threats = []
                ...
                return {
                    "status": "success",
                    "scan_type": "quick_pattern",
                    "privacy_assessment": {
                        "entities_detected": len(threats),
                        "entity_types": threats,
                        "privacy_risk_score": len(threats) * 2,
                        "compliance_violations": ["GDPR: Personal data detected"] if threats else [],
                        "recommendations": ["Apply data redaction"] if threats else ["No PII detected"]
                    },
                    "processing_available": False,
                    "fallback_mode": True,
                    "plugin_version": "1.0.0_quick"
                }
            
            # Try quick scan first for immediate response
            if len(text) > 1000:  # For long text, use quick scan
                return quick_pii_scan(text)
                
            # For normal text, continue with original logic but with timeout awareness  
            scan_type = ctx.get('scan_type', 'input')  # input or output
            context = ctx.get('context', {})
            
            # Try to initialize DLP plugin with timeout protection
            try:
                # Initialize DLP plugin
                dlp_plugin = PresidioDLPPlugin(cfg)
            except Exception as e:
                # Fallback to quick scan if initialization fails
                return quick_pii_scan(text)
            
            # Execute privacy assessment with timeout protection
            try:
                # NOTE: These should be sync calls, not async
                assessment = dlp_plugin.assess_privacy_risk(text, context)
                
                # Generate security threats  
                if scan_type == 'output':
                    threats = dlp_plugin.scan_output(text, context)
                else:
                    threats = dlp_plugin.scan_input(text, context)
                ...
                return {
                    ...
                    "privacy_assessment": {
                        ...
                        "entities_detected": len(assessment.detected_entities),
                        "entity_types": list(set(e.entity_type for e in assessment.detected_entities)),
                        "privacy_risk_score": assessment.privacy_risk_score,
                        "compliance_violations": assessment.compliance_violations,
                    ...
                    "detected_entities": [
                        {
                            "entity_type": entity.entity_type,
                            "text_preview": entity.text[:10] + "..." if len(entity.text) > 10 else entity.text,
                            "confidence": entity.confidence,
                            "start": entity.start,
                            "end": entity.end,
                            "recognizer": entity.recognition_metadata.get('recognizer', 'unknown')
                    ...
                    "processing_available": PRESIDIO_AVAILABLE,
                    "plugin_version": plug_metadata["version"],
                    "ai_enabled": dlp_plugin.enable_llm,
                    "entities_found": len(assessment.detected_entities)
                }
            except Exception as e:
                # Fallback to quick scan on processing error
                return quick_pii_scan(text)
    ...
    # Run with timeout protection using ThreadPoolExecutor
    import concurrent.futures
    try:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(_presidio_process)
            return future.result(timeout=10.0)
    except concurrent.futures.TimeoutError:
        # Quick fallback on timeout with robust parameter parsing
        text = ctx.get('text', ctx.get('payload', ctx.get('content', '')))
        import re
        threats = []
        ...
        return {
            ...
            "message": "Presidio timed out, using quick pattern matching",
            "privacy_assessment": {
                "entities_detected": len(threats),
                "entity_types": threats,
                "privacy_risk_score": len(threats) * 2,
                "recommendations": ["Apply data redaction"] if threats else ["No immediate threats detected"]
            },
