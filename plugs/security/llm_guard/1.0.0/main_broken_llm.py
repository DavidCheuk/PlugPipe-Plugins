...
def process(ctx, cfg):
    """
    PlugPipe entry point for LLM Guard security plugin with timeout protection
    ...
    async def _llm_guard_process():
        try:
            ...
            # Quick pattern-based fallback for immediate response
            def quick_threat_scan(text):
                import re
                threats = []
            ...
            # Use quick scan for long text or if LLM Guard not available
            if not LLM_GUARD_AVAILABLE or len(text) > 2000:
                return quick_threat_scan(text)
            
            # Try to initialize plugin with timeout protection
            try:
                plugin = LLMGuardPlugin(cfg)
                
                if not plugin.enabled:
                    return quick_threat_scan(text)
                
                # Perform security scan based on operation
                if operation == 'scan_input':
                    threats = await plugin.scan_input(text, scan_context)
                elif operation == 'scan_output':
                    threats = await plugin.scan_output(text, scan_context)
                else:
                    return {
                        "status": "error",
                        "error": f"Unknown operation: {operation}. Use 'scan_input' or 'scan_output'"
                    }
                ...
                # Fallback to quick scan on error
                return quick_threat_scan(text)
    ...
    try:
        def run_llm_guard_safely():
            return asyncio.run(_llm_guard_process())
        
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(run_llm_guard_safely)
        ...
        if re.search(r'(?i)ignore.*(previous|instructions)', text):
            threats.append({
                "scanner": "timeout_fallback",
                "threat_type": "prompt_injection",
                "risk_score": 0.6,
                "is_valid": False,
                "reason": "Quick pattern detection (timed out)"
            })
        
        return {
            "status": "timeout_fallback",
            "message": "LLM Guard timed out, using quick pattern matching",
            "threats_detected": threats,
        ...
        return {
            ...
            "llm_guard_available": True,
            "scanners_active": {
                "input_scanners": len(plugin.input_scanners),
                "output_scanners": len(plugin.output_scanners)
            }
        }
