# Patient Scheduling Pipe Examples

This directory contains example configurations and usage patterns for the patient_scheduling pipe.
...
scheduling_system:
  # Configure scheduling_system plugin
...
notification_service:
  ...
  timeout: 60
patient_management:
  # Configure patient_management plugin
  api_key: your_api_key_here
  timeout: 60
```
...
scheduling_system:
  # Advanced scheduling_system configuration
  api_key: your_api_key_here
  timeout: 120
  retry_attempts: 3
  rate_limit: 10
...
scheduling_system:
  api_key: your_scheduling_system_api_key
  timeout: 60
  retry_attempts: 3
...
python scripts/orchestrator_cli.py run \
  --pipeline pipes/workflow/patient_scheduling/pipe.yaml \
  --config config.yaml
...
async def run_workflow_with_monitoring():
    # Pre-execution setup
    workflow_id = f"patient_scheduling_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
tail -f pipe_runs/*/logs/patient_scheduling.log
...
python scripts/pipe_validator.py pipes/workflow/patient_scheduling/pipe.yaml

# Test plugin connectivity
python -c "from shares.loader import pp; print(pp(\"scheduling_system\", {\"action\": \"health_check\"}))"
python -c "from shares.loader import pp; print(pp(\"notification_service\", {\"action\": \"health_check\"}))"
python -c "from shares.loader import pp; print(pp(\"patient_management\", {\"action\": \"health_check\"}))"
```
